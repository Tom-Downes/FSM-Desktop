...
class App(AssetMixin, tk.Tk):
    ...
    def _build_header(self, parent):
        ...
        for label_txt, var_name, mod_name, setter, lo, hi in [
            ...
            tk.Label(fr, text=label_txt, bg=T.BG_CARD, fg=T.TEXT_DIM,
                     font=T.F_TINY).pack()
    ...
    def _refresh_madness_display(self):
        ...
        for i, m in enumerate(self.state.madnesses):
            ...
            self._madness_lb.insert("end", f"  {name}")
            fg = m.kind_color
    ...
    def _show_madness_effect(self, kind: str, roll_range: str, name: str, effect: str):
            ...
            if i <= 0 or tok != self._mad_fx_tok:
                if hasattr(self, "_madness_effect_current_kind"):
                    ...
                    for w in [self._madness_effect_text, self._madness_effect_title,
                              self._madness_effect_roll, self._madness_effect_desc]:
                        if hasattr(w, "config"):
                            w.config(bg=bg)
    ...
    def _wound_encounter(self):
        ...
        self._wound_roll_lbl.config(
            text=f"CON Save: d20({roll}) + CON({self.state.con_mod:+d}) = {total}  vs DC {dc}")
    ...
    def _encounter(self):
        ...
        self.roll_lbl.config(
            text=f"WIS Save: d20({d20}) + WIS({self.state.wis_mod:+d})"
        ...
        self._log(f"🎲 WIS Save: d20({d20}) + {self.state.wis_mod:+d}"
